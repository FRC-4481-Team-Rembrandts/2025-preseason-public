...
public class SimpleMotorSim {
    ...
    private double positionToVelocityRatio;
    ...
    private MechanismLigament2d motorMech;
    ...
    private void createMechanism2d(
        ...
        MechanismRoot2d root = mech.getRoot("root", x, y);
        motorMech = root.append(new MechanismLigament2d("motor", length, zeroAngle.getDegrees()));
    ...
    private void update() {
        dT = (Logger.getTimestamp() / 1e6) - lastUpdateTime;
        if (dT < 1e-4) {
            return;
        }
        lastUpdateTime = Logger.getTimestamp() / 1e6;
