...
public class PWMLEDController {
    ...
    /**
     * Adds an LED strip to the controller. This is a relatively expensive call, so it should be avoided in the loop. If
     * a new strip is added between two existing strips, all strips after the new strip will have their index shifted.
    ...
    /**
     * Adds a new LED strip to the controller with a certain length and offset from the start. This is a relatively
     * expensive call, so it should be avoided in the loop. If a new strip is added between two existing strips, all
     * strips after the new strip will have their index shifted. But the lengths and offsets will be unaffected.
     *
    ...
    /**
     * Adds a new LED strip to the controller with a certain length right after the last strip. This is a relatively
     * expensive call, so it should be avoided in the loop.
     *
     * @param length The length of the strip in LEDs.
     * @throws DuplicateLEDAssignmentException If the new strip overlaps with an existing strip.
     */
    ...
    public void addStrip(int length) throws DuplicateLEDAssignmentException {
        int offset = 0;
        if (!strips.isEmpty()) {
            offset = strips.get(strips.size() - 1).getOffset()
                    + strips.get(strips.size() - 1).getLength();
        }
    ...
    /**
     * Removes the LED strip at a certain index. This is a relatively expensive call, so it should be avoided in the
     * loop. If the strip that gets removed is not the last strip, all strips after the removed strip will have their
     * index shifted.
     *
    ...
    private void updateTotalStripLength() {
        int totalLength = 0;
        if (!strips.isEmpty()) {
            totalLength = strips.get(strips.size() - 1).getOffset()
                    + strips.get(strips.size() - 1).getLength();
        }
