...
/**
 * A space pattern that can be applied to an LED strip. This pattern will create a space-like pattern on the LED strip.
 *
...
public class SpacePattern implements LEDPattern {
    ...
    public void updateBuffer(AddressableLEDBuffer buffer, LEDStrip strip) {
        ...
        double[] hue = IntStream.range(0, ledLength)
                .mapToDouble(i -> periodicFunc(
                        numbers,
                        IntStream.range(0, maxk)
                                .mapToDouble(j -> amplitudesList[j][t])
                                .toArray(),
                        IntStream.range(0, maxk)
                                .mapToDouble(k -> shiftsList[k][t])
                                .toArray(),
                        i / (double) ledLength))
                .toArray();
    ...
    private static double[] periodicFuncRand(Random rng, int maxk, double scale, int n) {
        ...
        double[] amplitudes =
                rng.doubles(maxk, -scale, scale).sorted().map(d -> -d).toArray();
        double[] shifts = rng.doubles(maxk, -Math.PI, Math.PI).toArray();
        return IntStream.range(0, n)
                .mapToDouble(i -> periodicFunc(numbers, amplitudes, shifts, i / (double) n))
                .toArray();
    }
}
