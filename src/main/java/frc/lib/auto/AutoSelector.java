...
public class AutoSelector {
    ...
    private static Object[] selectSubsystems(List<SubsystemBase> subsystemList, Method method) {
        ...
        for (int i = 0; i < parameterTypes.length; i++) {
            ...
            for (Object obj : tempSubsystemList) {
                if (parameterTypes[i].isInstance(obj)) {
                    argsForMethod[i] = obj;
                    tempSubsystemList.remove(obj); // Remove object from the list to make sure same object is not passed twice
                    break;
                }
            }
        }
        ...
        for (Object obj : argsForMethod) {
            if (obj == null) {
                throw new IllegalArgumentException("Autonomous routine [" + method.getName() + "] has missing parameters, " +
