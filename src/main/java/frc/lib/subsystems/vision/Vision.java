...
public class Vision extends SubsystemBase {
    ...
    public Command processVision(Supplier<Pose2d> robotPoseSupplier, Consumer<VisionMeasurement> measurementConsumer){
        return Commands.run(() -> {
            ...
            for (int i = 0; i < visionIOs.length; i++){
                //Check for results, the hasResults is also false if there is something wrong with the camera object
                if (!inputs[i].hasResults){ continue; }

                //If ambiguity ratio is too high, continue to the next IO
                if (inputs[i].ambiguityRatio > filterParameters.maxAmbiguityRatio()){ continue; }
                ...
                Matrix<N3,N1> stdDevMat = determineStandardDeviation(tagDistance, inputs[i].fieldSpaceRobotPoses.length > 1);

                //Retrieve the timestamp of the measurement
                double timestamp = inputs[i].timeStamp;

                //Add the measurement to the queue
                measurementConsumer.accept(new VisionMeasurement(selectedPose.toPose2d(), timestamp, stdDevMat));
            }
    ...
    private Pose3d selectClosestPose(Pose3d[] fieldSpaceRobotPoses, Pose2d actualRobotPose){
        //Get the distance closest to the current robot pose if multiple poses are determined for a camera
        double minDistance = Double.POSITIVE_INFINITY;
        Pose3d selectedPose = null;

        for (Pose3d fieldSpaceRobotPose : fieldSpaceRobotPoses) {
            Pose2d measuredPose2d = fieldSpaceRobotPose.toPose2d();
            double distance = measuredPose2d.getTranslation().getDistance(actualRobotPose.getTranslation());
    ...
    private boolean outsideFieldBounds(Pose3d selectedPose){
        return selectedPose.getTranslation().getX() < 0 ||
                selectedPose.getTranslation().getX() > filterParameters.fieldLength().in(Units.Meters) ||
                selectedPose.getTranslation().getY() < 0 ||
                selectedPose.getTranslation().getY() > filterParameters.fieldWidth().in(Units.Meters) ||
                selectedPose.getTranslation().getZ() < -filterParameters.zMargin().in(Units.Meters) ||
                selectedPose.getTranslation().getZ() > filterParameters.zMargin().in(Units.Meters);
    }
    ...
    private double calculateAverageTagDistance(double[] tagAreas){
        ...
        double minTagDistance = filterParameters.aprilTagWidth().in(Units.Meters) / (2 * Math.tan(filterParameters.estimatedFOV().getRadians()/2));
